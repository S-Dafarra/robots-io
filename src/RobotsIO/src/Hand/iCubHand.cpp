    ...
    const bool &use_abduction)
{
    ...
    if (use_analogs)
    {
        ...
        use_analogs_bounds = inner_rf.check("use_bounds", Value(false)).asBool();
        if (use_analogs_bounds)
        {
            ...
            analog_bounds.setCol(0, bounds_col_0);
            analog_bounds.setCol(1, bounds_col_1);
        }
    }

    setup(robot_name, laterality, port_prefix, use_analogs, use_analogs_bounds, analog_bounds, thumb_version, use_abduction);
}
...
void iCubHand::setup(const std::string &robot_name, const std::string &laterality, const std::string &port_prefix, const bool &use_analogs, const bool &use_analogs_bounds, const yarp::sig::Matrix &analog_bounds, const std::string &thumb_version, const bool &use_abduction)
{
    /* Check YARP network. */
    ...
    if (use_analogs_)
    {
        use_bounds_ = use_analogs_bounds;
        if (use_bounds_)
        {
            if (analog_bounds.rows() != 15 || analog_bounds.cols() != 2)
            {
                throw std::runtime_error(log_name_ + "::ctor. use_analogs_bounds is set to true, but analog_bounds is not a 15x2 matrix.");
            }

            analog_bounds_ = analog_bounds;
        }
